...
cipherAES = Cipher(algorithms.AES(test_key),
                   modes.ECB(),
...
for index,kat in enumerate(nist_kats):
    cleartext,want_ciphertext = kat
    cleartext_bytes = bytes.fromhex(cleartext)
    ciphertext_bytes = aesEncryptor.update(cleartext_bytes)
